...
\begin{document}
\title{Universidad Simón BolÃ­var \\ Lenguajes de Programación II \\ Zinara}
\author{
  Daniel Barreto - \#04-36723 \texttt{<daniel.barreto.n@gmail.com>} \\
  GermÃ¡n Jaber - \#06-39743 \texttt{<germy-boy@gmail.com>}
}
\maketitle
...
\label{chap:intr}
El presente trabajo explica el desarrollo e implementación de un
lenguaje de programación titulado
\emph{\textbf{Zinara}}\footnote{\emph{Zinara Is Not A Recursive
    Acronym...}} el cual se realiza como parte de un proyecto
academico de lenguajes, blah
...
\subsection{Tipos bÃ¡sicos}
\label{sec:tiposbasicos}

Los tipos bÃ¡sicos del lenguaje son:
...
Sintaxis:
\begin{verbatim}
  asig      -> Â«idÂ» asig_list Â«exprÂ» 
  asig_list -> ',' Â«idÂ» asig_list Â«exprÂ» ','
            -> '='
\end{verbatim}

De igual forma que la declaración, la asignación puede ser aplicada
tanto a un único identificador como a una secuencia de
identificadores.
...
Imprime la expresion \texttt{<expr>} a la salida est\'andar.
...
definiendo la sintaxis en forma de gramÃ¡tica, si no de manera mÃ¡s natural.
...
Esta el la estructura para crear ciclos mÃ¡s simple de todo el lenguaje.//
...
'on' probando el valor de la expresión, tomarÃ¡ la primera que sea cierta
y cuando termine su ejecución volverÃ¡ a recorrer las clausulas. La
...
Sintaxis:
\begin{verbatim}
   for Â«idÂ» in Â«exprÂ» :
       Â«codeÂ»
   end
\end{verbatim}

Esta estructura esta diseñada para recorrer listas y tuplas del
lenguaje, por lo que Â«exprÂ» sólo puede ser una tupla o lista. El
elemento actual de la iteración solo se puede accesar a travÃ©s del
identificador Â«idÂ» dentro del ciclo. Ni el elemento iterado (Â«exprÂ») ni
el Ã­ndice (Â«idÂ») pueden ser modificados dentro de la estructura de control.\\
...
\begin{itemize}
\item \texttt{and}, \texttt{\&}, \texttt{or}, \texttt{|}, \texttt{?}
...
+      &  Int  & Float & Char  & Bool  \\
	\hline \hline
Int    & Int   & Float & Error & Error \\
Float  & Float & Float & Error & Error \\
Char   & Error & Error & Error & Error \\
...
*      &  Int  & Float & Char  & Bool  \\
	\hline \hline
Int    &   X   & Float & Error & Error \\
Float  & Float &   X   & Error & Error \\
Char   & Error & Error &   X   & Error \\
Bool   & Error & Error & Error &   X   \\
	\hline
\end{tabular}
\\
\begin{tabular}{|c||cccc|}
...
\item \textbf{Expresión Binaria:} Es una expresion que está
  ...
  binarios detallados en la sección \ref{sec:operadores}.
...
\item \textbf{Expresión Unaria:} Es una expresión que se encuentra
  ...
  en la sección \ref{sec:operadores}.
...
\begin{verbatim}
def <identificador>(<lista\_de\_argumentos>) return <<tipoR>>:
...
\item Ambas herramientas están bien documentadas y ofrecen facilidades
...
\subsection{Árbol sintÃ¡ctico abstracto}
\label{sec:ast}
...
las secciones \ref{sec:expr} y \ref{sec:inst} respectivamente.\\
